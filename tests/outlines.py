...
def scenario_with_examples_using_outline_with_args(self):
    ...
    for example in self.examples:
        Example(name=example, test=scenario_outline_with_args, type=self.type)(**vars(example))
        break

@TestScenario
def scenario_with_inner_examples(self):
    if not self.examples:
        self.examples = Examples("name value", [
                ("hi","there"),
                ("bye", "here")
            ])
    for example in self.examples:
...
with Module("regression"):
    ...
    Scenario(run=scenario_with_examples_using_outline_with_args, 
    ...
    Scenario("my scenario", test=scenario_outline_with_examples_and_args)(name="hello", value="there")
    Scenario("my scenario", test=scenario_outline_with_examples_and_args, examples=None)(name="hello", value="there")
    Scenario("my scenario", test=scenario_outline_with_examples_and_args)()
    Scenario("run scenario outline that has examples and args", 
        run=scenario_outline_with_examples_and_args)
    Scenario("run scenario outline that has examples and args and specify one arg",
        run=scenario_outline_with_examples_and_args, args={"name":"hello"})
    Scenario("test scenario outline that has examples and specify test and call args", 
        test=scenario_outline_with_examples_and_args, args={"name":"hello"})(value="there")
