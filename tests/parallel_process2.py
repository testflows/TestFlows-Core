...
async def my_async_test(self):
    async with Scenario("multiple parallel async tests"):
        with ProcessPool(5) as pool:
            ...
            for i in range(10):
                Scenario(name=f"test {i}", test=simple_test, flags=XFAIL, parallel=True, executor=pool)()

        await join()

@TestScenario
def my_sync_test(self):
    with Scenario("multiple parallel sync tests"):
        with ProcessPool(100) as pool:
            # f = Scenario(name=f"test 0", test=my_scenario, parallel=True, executor=pool)()
            # assert isinstance(f, asyncio.Future), error()
            # r = await f
            # assert r.result.value == "value", error()
...
def suite(self):
    ...
    #             assert len(test.futures) == 0, error()
        ...
        #with Scenario("sync test that runs parallel tests in a process pool"):
    Scenario(test=my_sync_test)()
...
@TestFeature
def feature(self):
    Feature(name=f"tests #0", test=suite)()
    return
    with ProcessPool() as pool:
        for i in range(1):
            Feature(name=f"tests #{i}", test=suite, parallel=False, executor=pool)()
        join()
