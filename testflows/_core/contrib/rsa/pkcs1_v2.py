...
from .pkcs1 import HASH_METHODS, compute_hash
from .transform import int2bytes
...
def mgf1(seed: bytes, length: int, hasher: str = 'SHA-1') -> bytes:
    ...
    try:
        hash_length = HASH_METHODS[hasher]().digest_size
    except KeyError:
        raise ValueError(
            'Invalid `hasher` specified. Please select one of: {hash_list}'.format(
                hash_list=', '.join(sorted(HASH_METHODS.keys()))
            )
        )
    ...
    output = b''.join(
        compute_hash(
            seed + int2bytes(counter, fill_size=4),
            method_name=hasher,
        )
        for counter in range(ceil_div(length, hash_length) + 1)
    )
