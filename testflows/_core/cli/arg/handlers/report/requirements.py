...
class Formatter:
    ...
    def format_table(self, data):
        ...
        for r in reqs.values():
            s += f'<span class="requirement requirement-inline"><i class="utf-icon {self.icon_colors[r["status"]]}">{self.utf_icons[r["status"]]}</i>{r["requirement"].name}</span><br>'
            description = r["requirement"].description.replace("\\n","\n")
            s += f'<div class="requirement-description hidden show" markdown=1>{description}</div>'
            for test in r["tests"]:
                result = test["result"]
